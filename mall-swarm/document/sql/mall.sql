...
INSERT INTO `pms_brand` VALUES (59, '测试品牌', 'C', 0, 0, 0, NULL, NULL, 'http://192.168.100.137:9000/mall/20220609/Snipaste_2022-06-08_14-35-53.png', 'http://192.168.100.137:9000/mall/20220609/biji_05.jpg', '12345');
...
CREATE TABLE IF NOT EXISTS art_article (
    ...
    sort         INT             DEFAULT 0,
    status       TINYINT         DEFAULT 0 COMMENT '0未发布,1已发布,2下架',
    is_top       TINYINT         DEFAULT 0 COMMENT '0否,1是',
    publish_time DATETIME        NULL,
    create_time  DATETIME        DEFAULT CURRENT_TIMESTAMP,
    update_time  DATETIME        DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    KEY idx_article_status   (status),
    KEY idx_article_publish  (publish_time DESC),
    KEY idx_article_category (category),
    KEY idx_article_is_top   (is_top DESC, sort ASC)
...
CREATE TABLE IF NOT EXISTS art_paper (
    ...
    KEY idx_paper_publish    (publish_time DESC),
    KEY idx_paper_category   (category),
    KEY idx_paper_doi        (doi),
    KEY idx_paper_pub_year   (publish_year DESC)
...
-- 3. 新闻表
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS art_news (
                                        id           BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
                                        title        VARCHAR(255)    NOT NULL,
    summary      TEXT,
    content      MEDIUMTEXT,
    ...
    KEY idx_news_publish    (publish_time DESC),
    KEY idx_news_category   (category),
    KEY idx_news_is_top     (is_top DESC, sort ASC),
...
-- 4. 内容订阅会员表
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS art_member (
                                          id           BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    update_time  DATETIME        DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    UNIQUE KEY uk_art_member_username (username),
    UNIQUE KEY uk_art_member_phone    (phone),
    UNIQUE KEY uk_art_member_email    (email),
    KEY idx_member_status             (status),
    KEY idx_member_expire             (expire_time),
    KEY idx_member_level_code         (level_code)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='内容订阅会员表';


-- ============================================================
-- 注：PG 版的 update_time 触发器在 MySQL 下由
--     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
--     原生能力替代，无需额外编写 TRIGGER。
-- ============================================================
...
-- 5.1 文章
INSERT IGNORE INTO art_article (id, title, summary, content, cover_image, author, category, tags, view_count, like_count, sort, status, is_top, publish_time)
VALUES
  (1, 'Spring Boot 3 快速入门', '从0到1搭建 Spring Boot 3 微服务，整合 Nacos、Sa-Token、MyBatis-Plus。', '详细内容略…', 'https://picsum.photos/seed/article1/600/360', '张三', '技术', 'SpringBoot,微服务,Sa-Token', 128, 22, 1, 1, 1, NOW() - INTERVAL 3 DAY),
  (2, 'PostgreSQL 迁移实战', 'MySQL → PostgreSQL 迁移指南：驱动、DDL、MyBatis XML、分页方言。', '详细内容略…', 'https://picsum.photos/seed/article2/600/360', '李四', '数据库', 'PostgreSQL,迁移,MyBatis', 356, 58, 2, 1, 0, NOW() - INTERVAL 1 DAY),
  (3, 'Kafka 订单超时取消方案', '对比死信队列 / Kafka + 定时任务 / 延迟队列三种方案，推荐 Kafka + 定时任务兜底。', '详细内容略…', 'https://picsum.photos/seed/article3/600/360', '王五', '架构', 'Kafka,订单,消息队列', 210, 41, 3, 1, 0, NOW());

-- 5.2 论文
INSERT IGNORE INTO art_paper (id, title, abstract_text, content, authors, keywords, journal, doi, pdf_url, publish_year, category, view_count, download_count, sort, status, publish_time)
VALUES
  (1, '基于 Transformer 的代码补全研究', '本研究使用…', '正文略', '赵六, 钱七', 'Code Completion, LLM', 'ACL 2025', '10.1234/example.1', 'https://example.com/paper1.pdf', 2025, '计算机/人工智能', 456, 89, 1, 1, NOW() - INTERVAL 60 DAY),
  (2, 'PostgreSQL 向量索引在 RAG 中的应用', '对比 pgvector 和 Milvus…', '正文略', '孙八', 'RAG, pgvector, 向量检索', 'SIGMOD Workshop', '10.1234/example.2', 'https://example.com/paper2.pdf', 2025, '计算机/数据库', 220, 34, 2, 1, NOW() - INTERVAL 30 DAY);

-- 5.3 新闻
INSERT IGNORE INTO art_news (id, title, summary, content, cover_image, source, author, category, view_count, sort, status, is_top, publish_time)
VALUES
  (1, 'mall-swarm 3.0 发布', '整合 AI 聊天室、代码生成、RAG、Kafka 消息队列、PostgreSQL。', '正文略', 'https://picsum.photos/seed/news1/600/360', '掘金', '技术编辑部', '行业动态', 1024, 1, 1, 1, NOW() - INTERVAL 5 HOUR),
  (2, 'Vercel AI Gateway 使用指南', '通过统一网关接入 OpenAI / Anthropic / Google / DeepSeek 多模型。', '正文略', 'https://picsum.photos/seed/news2/600/360', 'InfoQ', '张三', '云服务', 512, 2, 1, 0, NOW() - INTERVAL 2 DAY);

-- 5.4 会员
INSERT IGNORE INTO art_member (id, username, phone, email, avatar, level, level_code, status, start_time, expire_time, balance, points, remark)
VALUES
  (1, 'alice',   '13800000001', 'alice@example.com',   'https://i.pravatar.cc/150?img=1', '黄金会员',  'GOLD',   1, NOW() - INTERVAL 180 DAY, NOW() + INTERVAL 186 DAY, 199.00, 1200, '长期订阅用户'),
  (2, 'bob',     '13800000002', 'bob@example.com',     'https://i.pravatar.cc/150?img=2', '白银会员',  'SILVER', 1, NOW() - INTERVAL 90 DAY,  NOW() + INTERVAL 90 DAY,   50.00,  580,  NULL),
  (3, 'charlie', '13800000003', 'charlie@example.com', 'https://i.pravatar.cc/150?img=3', '普通会员',  'FREE',   1, NOW(),                   NOW() + INTERVAL 7 DAY,     0.00,   20,  '新用户7天试用'),
  (4, 'diana',   '13800000004', 'diana@example.com',   'https://i.pravatar.cc/150?img=4', '钻石会员',  'DIAMOND',1, NOW() - INTERVAL 365 DAY, NOW() + INTERVAL 365 DAY, 688.80, 5400, '企业定制');
